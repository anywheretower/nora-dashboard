...
|---|---|---|
| `paso 4` | Prompt listo, esperando generación text2img | `comfy-text2img.mjs` |
| `paso 5` | Imagen generada, esperando revisión | Revisión humana o iteración |
...
**Total**: 139+ imágenes etiquetadas.
...
| `id` | int | PK autoincremental |
| `created_at` | timestamp | Fecha de creación |
| `Marca` | text | ⚠️ Con M mayúscula (inconsistencia legacy) |
...
| `descripccion` | text | ⚠️ Con doble C (typo legacy). Descripción del item |
...
Fechas relevantes para generación proactiva de creatividades. 3 niveles de herencia.
...
**Herencia**:
- `marca=null` + `rubro=null` → aplica a TODAS las marcas
- `marca=null` + `rubro="salud"` → aplica a marcas del rubro
- `marca="Equos"` → exclusiva de esa marca
...
- Dashboard: `creatividades/skills-dashboard.html`
